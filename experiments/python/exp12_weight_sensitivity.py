...
from experiments.python.exp11_real_world_corpus_benchmark import (
    DEFAULT_CORPUS_DIR,
    build_query_records,
...
def evaluate_weights(corpus_dir: str, max_chunks: int, query_stride: int, top_k: int, verbose: bool) -> dict:
    ...
    # OPTIMIZATION: Pre-encode all query texts and context in batch
    query_texts = [q.text for q in queries]
    context_befores = [q.context_before for q in queries]
    context_afters = [q.context_after for q in queries]
    ...
    # Batch encode query texts
    for batch_start in range(0, len(query_texts), batch_size):
        ...
        all_q_semantics.extend(batch_semantics)
    
    # Pre-compute states for all queries
    for i, query in enumerate(queries):
        state_a = context_to_state(encoder, projector, query.context_before, query.text)
        state_b = context_to_state(encoder, projector, query.context_after or query.context_before, query.text)
        all_q_states_a.append(state_a)
        all_q_states_b.append(state_b)
    
    if verbose:
        log(f"Pre-encoded {len(all_q_semantics)} queries and their states")
    ...
    for preset in WEIGHT_PRESETS:
        ...
        for state_idx, state_name in enumerate(states):
            ...
            # Use pre-encoded queries and states
            query_states = all_q_states_a if state_idx == 0 else all_q_states_b
            query_emotionals = [encoder.encode_emotional(s) for s in query_states]
            query_states_norm = [encoder.encode_state(s) for s in query_states]
            
            for query_idx, query in enumerate(queries, start=1):
                relevant = relevant_ids_for_query(store, query)
                q_sem = all_q_semantics[query_idx - 1]
                q_emo = query_emotionals[query_idx - 1]
                q_state = query_states_norm[query_idx - 1]
